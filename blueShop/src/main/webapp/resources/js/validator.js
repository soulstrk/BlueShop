...
function validateCheckId() { //아이디 유효성 실시간 체크
	var ide = document.getElementById('mbId').value; //입력된 아이디 값
...
function validateCheckPwd() { //비밀번호 유효성 검사
...
function validateCheckEqualPwd() { //비밀번호 재확인
...
function phoneChk() {
	var phoneFrontNumber = ["010", "016", "019"];
	var phone = document.getElementById('phone').value; //입력된 아이디 값
	var bool = regxPhone.test(phone);
	var frontNumber3 = phone.substring(0, 3);
	
	if (bool === true){
		return phoneFrontNumber.indexOf(frontNumber3) !== -1 ? true : false;
	} else if (bool === false){
		return false;
	}
}
